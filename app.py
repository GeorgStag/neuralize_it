...
class neuralize_it(App):
    ...
    def build(self):
        ...
        plt.plot( [random.random(), random.random(),random.random(), random.random(), random.random()], [random.random(), random.random(),random.random(), random.random(), random.random()] )
        plt.title('Welcome to neuralize_it!!!')
        plt.ylabel('random')
        plt.xlabel('random')
        #plt.show()
        plt.close()

        self.icon = 'extra/logo.jpg'
        ...
        intro = Label(
            ...
            size_hint = (.2, .15) )
        logo = Image( source='extra/logo.png',
        ...
        #train_b.bind(on_press=self.train_starts)
        ...
        train_l = Label(
            text="Set Training inputs: \noptimizer, loss function, \nmetrics, batch_s, epochs",
            size_hint = (.2, .15) )
        self.train_i = TextInput(multiline=True, size_hint = (.2, .15), text = "Adam, mse, mae, 20, 10")
    ...
    def pop_up_about(self, instance):
        with open('extra/about.txt') as f:
            txt = f.read()
    ...
    def train_starts(self, instance):
        loaded_label = Label(text="Please wait for the model to train... \n \nA new message will appear on top when training is finished... \n \nIf training takes too much time, please think of reducing the epochs...")
        pop_data = Popup(title="Training Started!!!", content=loaded_label, size_hint=(None, None), size=(600, 600))
        pop_data.open()
    ...
    def neural_uml(self, instance):
        try:
            plot_model(self.model, to_file='umls/neural1.png', show_layer_activations=True, show_shapes=False,
                                      show_layer_names=False, rankdir="LR")
            #visualkeras.layered_view(self.model, font=ImageFont, legend=False, draw_volume=False, spacing=50, to_file='umls/neural2.png')
            uml = Image( source='umls/neural1.png' )
    ...
    def tf_options(self, instance):
        with open('extra/options.txt') as f:
            txt = f.read()
